...
var fragmentShaderSource = `
	...
	{
		float g = 2.4;
		vec4 gamma = vec4(1/g, 1/g, 1/g, 1.0);
		vec4 inv_gamma = vec4(g, g, g, 1.0);

		vec4 sampled = vec4(1.0, 1.0, 1.0, texture(textur, TexCoords).r);
		vec4 sampled_linear = pow(sampled, inv_gamma);

		vec4 color_linear = pow(Color, inv_gamma);

		vec4 blended = color_linear * sampled_linear;
		color = pow(blended, gamma);
	}
`
